...
if [ $# -ne 12 ]; then
   echo """Usage: ./kill.sh -<option1> <value1> ...
    -i IP address
   ...
   echo "Example: ./kill.sh -i 192.168.1.X -u root -p raspberry -s 22 -d KEY -f http://host:port/xxx/xxx?api_key=xxx"
else
  while getopts ":i:u:p:s:d:f:" opt; do
    ...
    ./stop.sh ${board_ip} ${board_username} ${board_password} ${ssh_port} ${device_id} ${front_end_host}
fi
